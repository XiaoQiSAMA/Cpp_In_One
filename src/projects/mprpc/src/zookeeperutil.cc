...
void global_watcher(zhandle_t *zh, int type, int state,
    ...
    if (type == ZOO_SESSION_EVENT) { // 回调的消息类型是和会话相关的消息类型
        if (state == ZOO_CONNECTED_STATE) { // zkclient和zkserve连接成功
            // 该独立线程会在client连接serve成功时返回sem信号，表示连接成功
